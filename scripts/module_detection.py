                    ...
                    for idx_batch in range(num_imgs):
                        ...
                        for i in range(output_dict['detection_boxes'][idx_batch].shape[0]):
                            if output_dict['detection_scores'][idx_batch] is None\
                                ...
                                box.append(int(round(output_dict['detection_classes'][idx_batch][i])))  #box[4]: class id
        ...
        with detection_graph.as_default():
            ...
            with tf.Session() as sess:
                ...
                while (self.isRun):
                    ...
                    t0 = time.time()
                    for k in range(num_imgs):
                        image_batch_np[k] = utils.resize_cv2(images_list[k],input_size)
                    t1 = time.time()
                    self.mprint('resize images time cost {}s'.format(t1 - t0))
...
if __name__ == '__main__':
    modelDir = '../input/tk_bird'

    net = NetThread(-1, 'thread_test_net', modelDir)
    net.begin()

    images_path = ['../input/test_pics/w20190805084121403_1.jpg', '../input/test_pics/w20190805084121403_1.jpg', '../input/test_pics/w20190805084121403_2.jpg', '../input/test_pics/w20190805084121403_3.jpg']
    images_list = []
